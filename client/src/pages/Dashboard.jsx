  ...
  if (error) return (
    <div className="min-h-screen bg-gradient-to-br from-[#0a0a0a] via-[#1a1a2e] to-[#16213e] p-8">
      <div className="max-w-7xl mx-auto flex flex-col items-center justify-center min-h-[60vh] gap-4">
        ...
        <button onClick={load} className="px-4 py-2 rounded-lg bg-gradient-to-r from-cyan-500 to-blue-500 text-white text-sm font-medium hover:opacity-90 transition-all flex items-center gap-2">
      ...
      <div className="max-w-7xl mx-auto p-6 lg:p-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-slate-400 text-sm">Track your progress, earn points, and become a top performer!</p>
          </div>
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <div className="flex flex-wrap items-center gap-6">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-2xl font-bold text-white">{pointsStats.total_points || 0}</p>
                  </div>
                </div>
              ...
              {pointsStats.points_to_next_milestone > 0 && pointsStats.points_to_next_milestone !== 50 && (
                <div className="px-4 py-2 rounded-xl bg-white/[0.05] border border-cyan-500/30">
                  ...
                  <p className="text-lg font-semibold text-cyan-400">{pointsStats.points_to_next_milestone} points</p>
                </div>
              )}
            </div>
          </div>
        ...
        {!pointsEnabled && (
          <div className="mb-6 p-4 rounded-xl bg-amber-500/10 border border-amber-500/20 text-center">
            <p className="text-amber-400 text-sm font-medium">✨ Points System Coming Soon!</p>
            <p className="text-slate-400 text-xs mt-1">Complete quizzes to earn points and track your progress</p>
          </div>
        )}

        {/* Stats Grid */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          {statCards.map(({ label, value, suffix, icon: Icon, color, gradient }) => {
            const c = colorMap[color];
            return (
              <div key={label} className={`relative overflow-hidden group rounded-2xl bg-gradient-to-br ${gradient} backdrop-blur-sm border border-white/[0.08] p-5 transition-all hover:scale-105 duration-300`}>
                <div className="relative z-10">
                  <div className={`w-10 h-10 rounded-xl ${c.bg} border ${c.border} flex items-center justify-center mb-3 group-hover:scale-110 transition-transform`}>
                    <Icon className={`w-5 h-5 ${c.text}`} />
                  </div>
                  <p className="font-display font-bold text-3xl text-white">
                    {value}
                    <span className="text-sm text-slate-400 font-body ml-0.5">{suffix}</span>
                  </p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-4">
            <div className="flex items-center justify-between">
              <div>
                <h2 className="font-display font-semibold text-white text-xl">Recent Quizzes</h2>
                <p className="text-slate-500 text-sm">Your latest quiz attempts</p>
              </div>
              <button
            ...
            {recentSessions.length === 0 ? (
              <div className="rounded-2xl bg-white/[0.03] border border-white/[0.08] p-12 text-center backdrop-blur-sm">
                ...
                <p className="text-white font-display font-semibold text-lg mb-1">No quizzes yet</p>
                <p className="text-slate-400 text-sm mb-6">Start your first MCQ quiz to test your knowledge</p>
                <button
                  onClick={() => navigate('/interview/new')}
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-3 mt-1">
                        ...
                        <p className="text-slate-500 text-xs flex items-center gap-1">
                          <BookOpen className="w-3 h-3" /> {s.total_questions} questions
                        </p>
                      </div>
                    </div>
            ...
            <div className="rounded-2xl bg-white/[0.03] border border-white/[0.08] p-5 backdrop-blur-sm mt-6">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="font-display font-semibold text-white">Quick Quiz Topics</h3>
              </div>
              <div className="grid grid-cols-2 gap-2">
                {[
                  { label: 'JavaScript', topic: 'JavaScript', icon: Zap, gradient: 'from-yellow-500/20 to-yellow-600/10', color: 'text-yellow-400' },
                  { label: 'React', topic: 'React, Hooks, State', icon: Target, gradient: 'from-blue-500/20 to-blue-600/10', color: 'text-blue-400' },
                  { label: 'Frontend', topic: 'HTML, CSS, JavaScript', icon: BarChart3, gradient: 'from-emerald-500/20 to-emerald-600/10', color: 'text-emerald-400' },
                  { label: 'Python', topic: 'Python, Django, Flask', icon: BookOpen, gradient: 'from-purple-500/20 to-purple-600/10', color: 'text-purple-400' },
          ...
          <div className="space-y-5">
            ...
            {scoreHistory.length > 0 && (
              <div className="rounded-2xl bg-white/[0.03] border border-white/[0.08] p-5 backdrop-blur-sm">
                ...
                <p className="text-slate-500 text-xs mb-4">Last {scoreHistory.length} completed quizzes</p>
                <div className="flex items-end gap-1.5 h-28">
                  {scoreHistory.map((s, i) => {
                    const pct = Math.max(5, Math.round((s.score / 100) * 96));
                    const fill = s.score >= 70 ? 'bg-emerald-500'
                               : s.score >= 50 ? 'bg-amber-400' : 'bg-rose-400';
                    return (
            ...
            <div className="rounded-2xl bg-gradient-to-br from-cyan-500/10 via-purple-500/10 to-pink-500/10 border border-white/[0.08] p-5 backdrop-blur-sm">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-white font-display font-semibold">Keep Practicing!</p>
