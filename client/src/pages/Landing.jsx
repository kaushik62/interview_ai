...
const features = [
  {
    icon: Brain,
    title: "AI Powered",
    desc: "Real-time intelligent feedback from Google's most capable AI model.",
  },
  {
    icon: Zap,
    title: "Instant Evaluation",
    desc: "Get scored answers with detailed strengths & improvement areas immediately.",
  },
  {
    icon: Target,
    title: "Role-Specific Questions",
    desc: "Tailored questions for your exact job role, experience level, and interview type.",
  },
  {
    icon: BarChart3,
    title: "Progress Tracking",
    desc: "Track your improvement across sessions with detailed analytics and charts.",
  },
];
    ...
    <div className="min-h-screen bg-ink-950 overflow-hidden">
      ...
      <section className="relative z-10 max-w-5xl mx-auto px-8 pt-24 pb-16 text-center">
        ...
        <p className="text-slate-400 font-body text-lg md:text-xl max-w-2xl mx-auto mb-10 leading-relaxed">
          Practice with an AI coach that gives real-time feedback, scores your
          answers, and helps you improve — tailored to your role and experience
          level.
        </p>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
          <button
            onClick={() => navigate("/register")}
            className="btn-electric flex items-center gap-2 text-base px-8 py-3.5"
          >
        ...
        <div className="flex flex-wrap justify-center gap-2 mt-12">
          {roles.map((r) => (
            <span
              key={r}
              className="bg-ink-800/80 border border-white/[0.07] rounded-full px-3 py-1 text-slate-400 text-xs font-body"
            >
              {r}
            </span>
          ))}
      ...
      <section className="relative z-10 max-w-5xl mx-auto px-8 py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {features.map(({ icon: Icon, title, desc }) => (
            <div
              key={title}
              className="glass-card p-6 group hover:border-electric-500/20 transition-all duration-300"
            >
      ...
      <section className="relative z-10 max-w-3xl mx-auto px-8 py-16 text-center">
        <div className="glass-card p-10 border-electric-500/15">
          ...
          <p className="text-slate-400 font-body mb-8">
            Join thousands of engineers preparing smarter with AI coaching.
          </p>
          <button
            onClick={() => navigate("/register")}
            className="btn-electric px-10 py-3.5 text-base"
          >
            Create free account
          </button>
        </div>
